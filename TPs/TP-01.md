...
Selon notre cours un mini-conteneur : rootfs + mount namespace + PID namespace + UTS + network + cgroups + process.
...
```bash
tar -xzf alpine-minirootfs-latest-x86_64.tar.gz -C /opt/manual-container/rootfs
```
Vérifier :
```bash
